...
Motivation: 在流体力学中，理解流动物理机制与准确预测同样重要，而传统机器学习方法缺乏可解释性。因此，研究旨在通过SR揭示复杂流动的数学关系，并结合ASP确保模型的物理合理性。

Method: 使用PySR库从数值模拟数据中生成符号方程，并结合ASP框架验证其物理合理性。
...
Method: I2I-STRADA通过模块化子任务建模分析推理的认知步骤，设计了一种结构化代理架构。
...
<details>
  <summary>Details</summary>
Motivation: 传统高级规划系统（APS）因高昂的咨询和维护成本难以普及，供应链规划师需要更易用的工具。

Method: 利用工具增强的大型语言模型构建对话系统，支持自然语言查询、反事实推理、推荐和场景分析。

Result: SmartAPS提供了一个直观的聊天界面，帮助规划师更高效地管理操作。

Conclusion: SmartAPS通过降低使用门槛，为供应链规划师提供了更易访问的APS解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 定性时间线规划的规划存在性问题已被证明是PSPACE完全的，但现有方法需要通过非确定性有限自动机进行转换，增加了复杂性。本文旨在找到一种可以直接映射到确定性有限自动机的方法，简化策略合成。

Method: 识别定性时间线规划的一个分段，将其规划存在性问题直接映射到确定性有限自动机的非空性问题，并确定适合该分段的Allen关系的最大子集。

Result: 成功识别了一个可以直接映射到确定性有限自动机的分段，并确定了适合该分段的Allen关系的最大子集。

Conclusion: 该方法简化了策略合成过程，避免了复杂的确定性转换步骤，为时间线规划提供了更高效的解决方案。
...
Method: 采用五阶段流程（错误生成、放大、精炼选择、转换、利用反馈），结合结构化提示、语义评分和人类评估。

Result: 创造力评分提升70.4%，口号更短、更新颖，跨模态测试表现优异，人类评价高分占比60%。

Conclusion: 错误驱动和反馈驱动的生成方法可增强AI创造力，为自进化、人类对齐的创意AI提供可行路径。
...
<details>
  <summary>Details</summary>
Motivation: 探究图表是否能为AI系统提供类似人类的帮助，提升数据分析能力。

Method: 使用GPT 4.1和Claude 3.5两种视觉语言模型，在三种分析任务中对比原始数据、空白图表和错误图表的性能。

Result: 伴随散点图时，AI系统描述数据更精确准确，尤其在复杂数据集中表现更优。
...
Result: 在43个任务和6种环境中，MAGPO表现优于基线CTDE方法，并匹配或超越完全集中式方法。
...
Result: 系统在20,000 GPU小时内完成1,773次实验，发现106种创新的线性注意力架构，性能超越人类设计。

Conclusion: ASI-Arch证明了科学发现的可扩展性，为AI系统的自我加速提供了蓝图。
...
<details>
  <summary>Details</summary>
Motivation: 解决医疗领域机器学习解决方案部署成本高、流程碎片化及数据隐私问题。
...
<details>
  <summary>Details</summary>
Motivation: 由于临床试验和早期广泛实施中安全数据收集窗口有限，需要建立许可后监测系统，以应对COVID-19疫苗的快速推广。

Method: 结合NLP、主动学习和数据增强技术，开发分类器，优化标注过程和数据质量。

Result: 分类器能够更准确、高效地从急诊科分诊笔记中识别疫苗安全问题，减少假阳性。

Conclusion: 该方法为疫苗安全监测提供了一种快速、高效的解决方案，尤其在数据稀缺的医疗领域具有应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 探索均值聚合GNNs的表达能力，并与求和和最大值聚合GNNs进行比较，以理解其在不同设置下的表现。

Method: 通过非均匀和均匀设置下的理论分析，比较均值聚合GNNs与模态逻辑和其他GNNs的表达能力。

Result: 非均匀设置下，均值GNNs与比率模态逻辑等价；均匀设置下，其表达能力低于求和和最大值GNNs。

Conclusion: 均值GNNs的表达能力在不同设置下表现不同，均匀设置下较弱，但在某些假设下可能增强。
...
<details>
  <summary>Details</summary>
Motivation: 受双系统认知理论启发，研究旨在区分大语言模型中知识与推理的作用，以提升模型分析、可解释性和开发能力。

Method: 通过快速思维（知识检索）和慢速思维（推理调整）两种认知模式，量化知识与推理的贡献，并在15个大语言模型和3个数据集上进行验证。

Result: 结果显示推理调整具有领域特异性，参数扩展显著提升知识能力，且知识主要存储于网络低层，推理操作集中于高层。

Conclusion: 该框架为理解大语言模型提供了新视角，并对扩展规律、知识编辑和小模型推理限制等研究提供了新见解。
...
<details>
  <summary>Details</summary>
Motivation: 探讨非最小化析取语义的替代方法，比较不同语义的异同及其与稳定模型的关系。

Method: 比较四种析取语义：Justified Models、Strongly Supported Models、Forks和Determining Inference (DI)语义，分析其定义和性质。

Result: 证明Forks、Justified Models和放松的DI语义实际上是同一方法，且强于Strongly Supported Models。

Conclusion: 这些非最小化析取语义为稳定模型提供了超集，且部分方法在定义上具有一致性。
...
<details>
  <summary>Details</summary>
Motivation: 解决AI部署中法律合规性与基本权利保护的复杂性。

Method: 采用定义性平衡（比例分析）和可废止推理，分析AI部署场景及其对基本权利的多层次影响。

Result: 提出了一个逻辑化的AI风险分析框架，适用于高风险AI和通用AI系统。
...
Conclusion: 提出的方法在批改物理考试中具有实际应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM推理基准多关注事实准确性或逐步逻辑，而金融领域需要模型在不确定性下生成创造性且合理的未来情景。

Method: ConDiFi包含发散性推理的宏观金融提示和收敛性推理的多跳对抗性选择题，评估了14个领先模型。
...
Method: 基于信息瓶颈原则，提出IBRO框架，推导出token级替代目标，并设计轻量级IB正则化方法。

Result: 在多个数学推理基准和RL算法上验证了IB正则化的有效性，显著提升了LLM推理性能。
...
<details>
  <summary>Details</summary>
Motivation: 优化呼叫中心的路由策略以减少客户等待时间和员工闲置时间。

Method: 基于模型的VI（已知系统动态）和基于模型的PPO（从经验中学习），均采用MDP框架和SBR模型。

Result: PPO在1000次测试中表现最佳，奖励最高，客户等待时间和员工闲置时间最低。
...
<details>
  <summary>Details</summary>
Motivation: 传统CPU在处理包含数百或数千有效案例的表约束时效率不足，需要利用GPU的大规模并行计算能力提升性能。

Method: 设计并实现GPU加速的CT算法，并将其集成到现有约束求解器中，通过实验验证其有效性。

Result: 实验验证表明，GPU加速的CT算法在处理大规模表约束时显著提升了性能。

Conclusion: GPU加速的CT算法为处理大规模表约束问题提供了高效解决方案，扩展了约束编程的应用范围。
...
<details>
  <summary>Details</summary>
Motivation: 概念探测用于解释神经网络，但研究多关注模型本身，忽略了训练数据的作用。本文填补了这一空白。

Method: 研究图像分类任务中训练数据对概念探测模型性能的影响，并提供两个数据集的标签。

Result: 分析了数据对探测模型性能的具体影响，并公开了数据集标签。
...
Result: SafeWork-R1在安全相关基准上平均提升46.54%，优于GPT-4.1和Claude Opus 4，同时保持通用能力。

Conclusion: SafeLadder框架证明安全与能力可以协同进化，适用于构建稳健、可靠且可信的通用AI。
...
<details>
  <summary>Details</summary>
Motivation: 探索多模态、长时程可变形物体操控的挑战，尤其是陶艺制作中精细动作的需求。

Method: 结合目标条件扩散策略模型、预训练3D点云嵌入、任务进度预测和碰撞约束动作投影。
...
<details>
  <summary>Details</summary>
Motivation: 设计一个能够在存在干扰和测量噪声的情况下确保机器人安全导航的MPC方案，填补理论NMPC与实际机器人应用之间的鸿沟。

Method: 采用非线性模型预测控制（NMPC）方法，逐步实现安全导航，包括状态和输入约束的满足以及避障。

Result: 提供了一种从理论到实现的实用路径，重点在于安全性和性能保证。

Conclusion: 该报告为研究人员和工程师提供了NMPC在机器人导航中的实际应用指南，并鼓励反馈以持续改进。
...
<details>
  <summary>Details</summary>
Motivation: 解决语言描述与机器人实际动作之间的差距，尤其是在开放世界中的复杂指令导航问题。

Method: 利用MLLMs的跨模态理解和代码生成能力，结合视觉语言感知模型生成2D鸟瞰图价值地图，整合语义与空间信息。

Result: 在户外导航任务中验证了零样本视觉语言导航框架的鲁棒性，并在室内外场景中展示了实际应用能力。

Conclusion: MLLMs能够有效提升机器人的导航能力，适用于开放世界的复杂指令任务。
...
<details>
  <summary>Details</summary>
Motivation: 解决由独立子系统组成的系统中状态和误差协方差估计的独立更新问题，同时处理多子系统状态依赖的相对测量。

Method: 采用模块化非线性最小二乘滤波方法，结合协方差交集算法防止信息重复计算，并在机器人-地标定位问题中具体应用。

Result: 通过随机模拟研究，验证了模块化方法相对于联合状态滤波器的性能权衡，并展示了在通信和带宽减少时的性能退化。

Conclusion: 提出的模块化方法在独立子系统系统中有效，协方差交集算法的整合避免了信息重复计算，适用于机器人-地标定位等耦合估计问题。
...
<details>
  <summary>Details</summary>
Motivation: 解决模型不匹配导致的性能下降问题，提升高不确定性环境中的控制性能和学习效率。

Method: 将残差模块与启发式设计的模型框架（如步态规划和动态模型）结合，并选择适合的学习方法。

Result: 在真实四足机器人中验证了框架的可行性，机器人能在不确定性环境中保持平衡和速度跟踪。
...
<details>
  <summary>Details</summary>
Motivation: 搜救任務需要高效且精確的技術來定位和追蹤特定個體，尤其是在複雜環境中。無人機結合AI技術可以提升任務的效率和準確性。

Method: 系統整合了ROS2框架，使用YOLOv11和YOLOv11-pose CNN進行追蹤，dlib庫進行人臉識別。通過系統識別和PD控制器實現自主導航。

Result: 初步實驗在14個已知個體上成功驗證了系統的實時性能。

Conclusion: 下一步將在大型實驗無人機上部署系統，並整合GPS導航以進一步優化搜救任務。
...
<details>
  <summary>Details</summary>
Motivation: 解决移动机器人在缺乏卫星或摄像头导航时的惯性导航漂移问题。

Method: 通过蛇形运动增加惯性信号信噪比，并结合物理约束的神经网络框架MoRPI-PINN。
...
Method: 使用机器人臂自动调整位置，固定手术灯和模型，测试MediaPipe算法在不同角度下的检测效果。

Result: 算法在大偏转角度下检测精度提高，但标准偏差增加。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在语义约束建模、实时规划和环境鲁棒性方面存在不足，限制了机器人操作的效率和适应性。

Method: ReSem3D采用分层递归推理，结合自然语言指令和RGB-D观测，分阶段构建3D空间约束，并将其编码为实时优化目标。

Result: 在丰富和稀疏语义环境中，ReSem3D在零样本条件下成功完成多样操作任务，表现出强适应性和泛化能力。

Conclusion: ReSem3D通过语义驱动的3D约束建模和实时优化，显著提升了机器人操作的效率和鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 铰接物体的内部结构不可直接观察，且几何多样性和功能机制差异导致统一的适应性操纵策略难以开发。

Method: 提出AdaRPG框架，利用基础模型提取部件级几何相似性，构建部件级功能标注数据集，并基于部件功能推理生成高级控制代码。

Result: 仿真和真实实验表明，AdaRPG在新铰接物体类别上表现出强大的泛化能力。

Conclusion: AdaRPG通过部件级分析和基础模型的结合，有效解决了铰接物体操纵的挑战。
...
Result: 在数据集和实际环境中测试，AF-RLIO在烟雾和隧道等挑战性条件下表现优于现有方法。

Conclusion: AF-RLIO通过多传感器融合有效提升了复杂环境中的机器人导航性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统3D椭球表示的各向同性不确定性在多视角下深度解释不一致，需要一种更几何感知的方法。

Method: 使用局部切平面的2D高斯盘表示场景元素，并将其嵌入广义ICP框架，引入几何感知损失监督光度、深度和法线一致性。
...
Method: 利用模仿学习学习三种操纵动作（高低加速度），并采用视觉分类器判断衣物状态。

Result: 动态动作对刚拆封衣物效果不佳，动作组合能显著改善展开效果。

Conclusion: 动作组合是高效展开衣物的关键，为机器人辅助穿衣提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 压缩感知在机器人环境监测中的应用潜力尚未充分挖掘，尤其是在优化采样轨迹方面。本文旨在填补这一空白。

Method: 提出蒙特卡洛优化框架，结合字典学习生成优化的CS测量矩阵，以减少机器人路径长度和重建误差。

Result: 实验表明，该方法将机器人路径缩短至全覆盖路径的10%以下，重建精度比传统CS方法提高五倍，比现有IPP方法提高两倍。

Conclusion: 该方法为机器人环境监测提供了一种高效的数据采集策略，显著提升了压缩感知在实践中的性能。
...
<details>
  <summary>Details</summary>
Motivation: 研究动机在于两种控制方法在理想条件下均能保证稳定性，但在实际应用中（如关节摩擦、传感器噪声等非理想条件下）的表现尚不明确。

Method: 通过实验比较两种控制器在人形机器人平台上的表现，包括摆动脚位置与方向控制、深蹲（带/不带额外重量）和跳跃任务。

Result: 实验揭示了两种控制器在不同任务中的性能差异，并分析了其优缺点。

Conclusion: 结论指出两种控制器各有优劣，具体选择需根据应用场景的需求决定。
